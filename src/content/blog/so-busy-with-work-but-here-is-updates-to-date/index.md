...
First experiments I am working towards are using IP WebCam (on Android) and Cambozola to produce something like the functionality of WOWWee Rovio (basic video streaming and remote control) before moving onto using OpenCV to experiment with flow optics.  OpenCV is slowly being ported (under open source by TI - see [here](http://code.google.com/p/opencv-dsp-acceleration/ "OpenCV to Beagleboard") and [here](http://www.ti.com/lit/wp/spry175/spry175.pdf "Word from TI (no promises)") otherwise go [here](http://processors.wiki.ti.com/index.php/Building_OpenCV_for_ARM_Cortex-A8 "Building for ARM Cortex-8") otherwise [here](http://opencv.willowgarage.com/wiki/Android "The easy way if you are on Android")) to the OMAP devices to accelerate it on the ARM side of the world - so no need to rush to install on Android just yet.
[QualCOMM Snapdragon S2 and above has an already tuned fast CV library (go [here](https://developer.qualcomm.com/develop/mobile-technologies/computer-vision-fastcv "See here if you have a Snapdragon")). Pity my phones are S1 :( ]
